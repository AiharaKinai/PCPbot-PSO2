		...
		if(args.command && !showAll) {
			if(commands.length === 1) {
				let help = stripIndents`
					${oneLine`
						...
						${commands[0].guildOnly ? ' (Usable only in  ー・完全犯罪党・ー)' : ''}
					`}
			...
			try {
				messages.push(await msg.direct(stripIndents`
					...
					\`\`\`${showAll ? 'All commands' : `Available commands in ${msg.guild || 'ー・完全犯罪党・ー'}`}\`\`\`
				...
				`, { split: true }));
				if(msg.channel.type !== 'dm') messages.push(await msg.reply('Sent you a :incoming_envelope:, please.. Check your :mailbox_with_mail:'));
			} catch(err) {
				messages.push(await msg.reply('Unable to send you the help DM. You probably have DMs disabled.'));
			}
			return messages;
