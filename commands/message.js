...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return await this.reply(`The \`${this.command.name}\` command must be used in ー・完全犯罪党・ー.`);
		}
		if(!this.command.hasPermission(this)) {
			this.client.emit('commandBlocked', this, 'permission');
			return await this.reply(`You don't have permissions to execute this \`${this.command.name}\` command!. :middle_finger:`);
		}
