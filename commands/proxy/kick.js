...
module.exports = class PSO2Commands extends Commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: "proxy",
            memberName: "kick",
            description: "[Admin Only] Kick/Disconnect Users from PCP Network.",
            examples: ["!kick <Player ID>"],
			guildOnly: true,
			 ...
			 args: [
                {
                    key: 'playerid',
                    label: 'param',
                    prompt: 'Type Player ID (Integer/Angka)',
                    type: 'string'
                }
            ]
        })
      ...
      async run(msg, args, client){
			...
			request(`http://172.105.227.34:9999/rcon?key=dudekeparat123&command=kick&params=${encodeURIComponent(playerid)}`, function (error, response, body) {
