...
module.exports = async (client) => {
    ...
    try {
        ...
        const response = await fetch('http://172.105.227.34:5000/eq2h/');
        ...
        const cache = JSON.parse(await fs.readFile("./cache2h.json"));


        if (data[0]["time"] !== cache["time"]) {

            await fs.writeFile("cache2h.json", `{ "time" : "${data[0]["time"]}" }`);

            const guilds = client.guilds.filter(guild => { return client.provider.get(guild, "alerts2h") });

            

            for (let guild of guilds) {

                let settings = await client.provider.get(guild[1], "alerts2h");
				...
				let embed = { embed: {
                            color: 10433828,
                            title: `Emergency Quest 【In Two Hours】`,
                            url: "http://pso2.jp/",
							description: "",
							footer: {
                ...
                if (eqs.length > 0 && eqs.length !== 10) {
                    ...
                    for (let eq of eqs) 
					{
							embed['embed']['fields'].push({name: `Ship ${eq['ship']}`, value:`${time.format("HH:mm")} GMT+7 | ${eq['name']}`, inline: true})
                    }
				}
				else 
				{
					 embed['embed']['fields'].push({name: "All Ships", value:`${time.format("HH:mm")} GMT+7 | ${eqs[0]['name']}`, inline: true})
				}
