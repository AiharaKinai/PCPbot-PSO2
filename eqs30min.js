...
module.exports = async (client) => {
    ...
    try {
        ...
        const response = await fetch('http://172.105.227.34:5000/eq30min/');
        ...
        const cache = JSON.parse(await fs.readFile("./cache30min.json"));


        if (data[0]["time"] !== cache["time"]) {

            await fs.writeFile("cache30min.json", `{ "time" : "${data[0]["time"]}" }`);

            const guilds = client.guilds.filter(guild => { return client.provider.get(guild, "alerts30min") });

            

            for (let guild of guilds) {

                let settings = await client.provider.get(guild[1], "alerts30min");
				...
				let embed = { embed: {
                            color: 10433828,
                            title: `Emergency Quest 【In 30 Minutes】`,
                            url: "http://pso2.jp/",
							description: "",
							footer: {
